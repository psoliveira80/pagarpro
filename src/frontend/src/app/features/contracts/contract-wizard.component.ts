...
@Component({
  ...
  styleUrl: './contract-wizard.component.css',
...
export class ContractWizardComponent {
  ...
  async onAssetSearch(term: string): Promise<void> {
    ...
    try {
      ...
      for (const v of (response as { items: Veiculo[] }).items) {
        ...
        opts.push({ value: v.id, label: `${v.placa} — ${v.marca} ${v.modelo}`, subtitle: `Ano ${v.ano_modelo} · ${v.cor}` });
      }
  ...
  async submit(): Promise<void> {
    ...
    try {
      ...
      this.router.navigate(['/system/contracts', contract.id]);
    } catch {
  ...
  goBack(): void {
    this.router.navigate(['/system/contracts']);
  }
