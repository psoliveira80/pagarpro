...
import { ClienteDashboardTabComponent } from '../cliente-dashboard-tab/cliente-dashboard-tab.component';

@Component({
  selector: 'app-cliente-detalhe',
  standalone: true,
  imports: [UiIconComponent, AtivosListaComponent, ClienteDashboardTabComponent, ModalComponent],
...
export class ClienteDetalheComponent implements OnInit {
  ...
  async exportCustomerData(): Promise<void> {
    ...
    try {
      const response = await fetch(`/api/v1/clientes/${c.id}/my-data`, {
  ...
  async requestAnonymization(): Promise<void> {
    ...
    try {
      const response = await fetch(`/api/v1/clientes/${c.id}/anonymize`, {
        method: 'DELETE',
        headers: {
